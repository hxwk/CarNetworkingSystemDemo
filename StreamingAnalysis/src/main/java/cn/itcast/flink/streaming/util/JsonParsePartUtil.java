...
public class JsonParsePartUtil {
     ...
     */
    public static ItcastDataPartObj parseJsonToObject(String jsonString) {
        ItcastDataPartObj itcastPartObj = new ItcastDataPartObj();
        try {
            ...
            if(!itcastPartObj.getTerminalTime().isEmpty()){
                itcastPartObj.setTerminalTimeStamp(DateUtil
                        .convertStringToDateTime(itcastPartObj.getTerminalTime(),DateFormatDefine.DATE_TIME_FORMAT).getTime());
                // 终端时间延迟五个小时，设置为默认值
